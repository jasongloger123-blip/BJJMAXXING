...
export default function GameplanPage() {
  ...
  const plan = (remotePlan as unknown as PlanConfig | null) ?? fallbackPlan
  const mainNodes = plan.mainPath.map((nodeId) => plan.nodes[nodeId]).filter(Boolean)
  const fallbackNode = mainNodes[0] ?? Object.values(plan.nodes)[0] ?? null
  ...
  const [planHeadline, setPlanHeadline] = useState(plan.headline)
  ...
  const shouldRenderSelectedPlan =
    !forceOverview && (hasSelectedPlan || (remotePlanLoaded && availablePlans.length === 1 && Boolean(remotePlan)))
  ...
  useEffect(() => {
    ...
    const preferredNodeId = remotePlan?.unlockSummary.currentNodeId ?? plan.mainPath[0]
  ...
  }, [plan.headline, plan.mainPath, remotePlan, remotePlanLoaded])
  ...
  const detailClips = useMemo(() => {
    ...
    return getCuratedClipsForNode(detailNode.sourceNodeId ?? detailNode.id)
  }, [connectionClipOverrides, detailEdge, detailNode])
  ...
  const visibleTreeIds = useMemo(() => {
    ...
    if (progressSummary) {
      return new Set(progressSummary.visibleNodeIds)
    }
    if (!detailOpen) {
      return new Set(plan.mainPath)
    }
    return getVisibleTreeIdsWithContext(activeNodeId, plan.nodes, plan.mainPath)
  }, [activeNodeId, detailOpen, plan.mainPath, plan.nodes, progressSummary, remotePlanLoaded, showAllNodes])
  ...
  function handleSelectPlan(plan: RemoteGameplan) {
    setForceOverview(false)
          ...
          {!hasSelectedPlan ? (
            ...
            <div className="h-full">
              ...
              >
                ...
                {progressSummary ? (
                  <div className="absolute left-1/2 top-6 z-20 w-[min(28rem,calc(100%-9rem))] -translate-x-1/2 px-2 text-white/88">
                          ...
                          <div
                            ...
                            style={{
                              width: `${Math.max(
                                8,
                                Math.round(
                                  ((progressSummary.coreCompletedCount + progressSummary.expansionCompletedCount) /
                                    Math.max(progressSummary.coreTotalCount + progressSummary.expansionTotalCount, 1)) *
                                    100
                                )
                              )}%`,
                            }}
                    ...
                    <div className="fluid-surface rounded-[1.25rem] bg-[linear-gradient(180deg,rgba(18,23,33,0.82),rgba(14,18,26,0.8))] p-3 shadow-[0_12px_26px_rgba(0,0,0,0.2)]">
                      ...
                      <div className="mt-4">
                        <GameplanClipDeck
                          ...
                          detailCtaLabel={detailEdge ? 'Mehr Details ansehen' : 'Technik oeffnen'}
                        />
                      </div>
                    </div>
                  </aside>
