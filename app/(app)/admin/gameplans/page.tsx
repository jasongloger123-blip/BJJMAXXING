...
import { ChevronRight, Copy, Link2, List, Minus, Plus, Save, Search, Settings, User, X, Users } from 'lucide-react'
...
export default function GameplanAdminPage() {
  ...
  const TabContent = () => {
    ...
    switch (activeTab) {
      case 'plans':
        return (
          <div className="space-y-3">
            <p className="text-xs font-black uppercase tracking-[0.24em] text-white/72">Plans</p>
            <label className="relative block">
              <Search className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-white/35" />
              <input 
                value={planSearch} 
                onChange={(event) => setPlanSearch(event.target.value)} 
                placeholder="Gameplan suchen..." 
                className="w-full rounded-2xl border border-white/10 bg-[#101723] py-3 pl-10 pr-3 text-sm text-white outline-none" 
              />
            </label>
            <div className="max-h-[60vh] space-y-2 overflow-auto pr-1">
              {filteredPlans.map((plan) => (
                <button 
                  key={plan.id} 
                  type="button" 
                  onClick={() => { setSelectedPlanId(plan.id); setSelectedTechniqueId(null) }} 
                  className={`w-full rounded-[1.25rem] border px-4 py-4 text-left ${plan.id === selectedPlanId ? 'border-bjj-gold/35 bg-bjj-gold/10' : 'border-white/10 bg-white/[0.03]'}`}
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="rounded-full bg-white/[0.08] px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-white/65">{plan.status}</span>
                  </div>
      ...
      case 'settings':
        return (
          <div className="space-y-3">
            <p className="text-xs font-black uppercase tracking-[0.24em] text-white/72">Settings</p>
            <select 
              value={selectedPlan.status} 
              onChange={(event) => updatePlan((plan) => ({ ...plan, status: event.target.value as 'draft' | 'published' }))} 
            ...
            <input 
              ...
              onChange={(event) => updatePlan((plan) => ({ ...plan, slug: event.target.value }))} 
              className="w-full rounded-2xl border border-white/10 bg-black/20 px-3 py-3 text-sm text-white outline-none" 
              placeholder="Slug" 
            />
            <input 
              value={selectedPlan.title} 
              onChange={(event) => updatePlan((plan) => ({ ...plan, title: event.target.value }))} 
              className="w-full rounded-2xl border border-white/10 bg-black/20 px-3 py-3 text-sm text-white outline-none" 
              placeholder="Titel" 
            />
            <input 
              value={selectedPlan.headline} 
              onChange={(event) => updatePlan((plan) => ({ ...plan, headline: event.target.value }))} 
              className="w-full rounded-2xl border border-white/10 bg-black/20 px-3 py-3 text-sm text-white outline-none" 
            ...
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                <button 
                  ...
                  className="rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-2 text-sm text-white/84"
                >
                  {uploadingHeroImage ? 'Laedt...' : 'Titelbild hochladen'}
                </button>
                {selectedPlan.heroImageUrl ? (
                  <button 
                    type="button" 
                    onClick={() => updatePlan((plan) => ({ ...plan, heroImageUrl: null }))} 
                    className="rounded-2xl border border-white/10 px-3 py-2 text-sm text-white/70"
                  >
                    Titelbild entfernen
                  </button>
                ) : null}
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 px-3 py-3 text-sm text-white/75">
              Hauptpfad: {deriveMainPath(selectedPlan).join(' -> ') || 'Noch keine A-Plan-Techniken'}
            </div>
          </div>
        )
      ...
      case 'technique':
        return (
          <div className="space-y-3">
            <p className="text-xs font-black uppercase tracking-[0.24em] text-white/72">Technik</p>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <select 
                value={pickerTrack} 
                onChange={(event) => setPickerTrack(event.target.value as typeof pickerTrack)} 
                className="rounded-2xl border border-white/10 bg-[#101723] px-3 py-3 text-sm text-white outline-none"
              >
                <option value="all">Tracks</option>
                <option value="foundation">Foundation</option>
                <option value="secondary">Secondary</option>
                <option value="top-game">Top Game</option>
              </select>
              <select 
                value={pickerStage} 
                onChange={(event) => setPickerStage(event.target.value as typeof pickerStage)} 
                className="rounded-2xl border border-white/10 bg-[#101723] px-3 py-3 text-sm text-white outline-none"
              >
              ...
              <select 
                value={pickerArchetype} 
                onChange={(event) => setPickerArchetype(event.target.value)} 
                className="rounded-2xl border border-white/10 bg-[#101723] px-3 py-3 text-sm text-white outline-none sm:col-span-2"
              >
                <option value="all">Alle Archetypen</option>
              ...
              <select 
                value={pickerSize} 
                onChange={(event) => setPickerSize(event.target.value as typeof pickerSize)} 
                className="rounded-2xl border border-white/10 bg-[#101723] px-3 py-3 text-sm text-white outline-none"
              >
                <option value="main">A-Plan gross</option>
                <option value="branch">B-Plan mittel</option>
                <option value="future">Extra klein</option>
              </select>
            </div>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-white/55">{node.subtitle}</p>
                      {node.recommendedArchetypeIds.length > 0 ? (
                        <p className="mt-2 text-[11px] text-bjj-gold">
                          {node.recommendedArchetypeIds.map((id) => archetypeNameById.get(id) ?? id).join(' • ')}
                        </p>
                      ) : null}
                    </div>
                    <span className="rounded-full bg-white/[0.08] px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-white/65">
      ...
      case 'connections':
        return (
          <div className="space-y-3">
            <p className="text-xs font-black uppercase tracking-[0.24em] text-white/72">Connections</p>
            <button 
              type="button" 
              disabled={!selectedTechnique} 
              onClick={() => selectedTechnique && startConnectMode(selectedTechnique.id)} 
              className="w-full rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-3 text-sm font-semibold text-white/84 disabled:opacity-40"
            >
              {connectMode && selectedSourceNodeId === selectedTechnique?.id ? 'Ziel-Technik auf der Canvas anklicken' : 'Connect Mode starten'}
            </button>
            {connectMode ? (
              <button 
                type="button" 
                onClick={cancelConnectMode} 
                className="w-full rounded-2xl border border-bjj-gold/25 bg-bjj-gold/10 px-3 py-3 text-sm font-semibold text-bjj-gold"
              >
                Connect Mode abbrechen
              </button>
            ) : null}
            <div className="max-h-[50vh] space-y-2 overflow-auto pr-1">
                ...
                <div key={edge.id} className="rounded-2xl border border-white/10 bg-black/20 p-3">
                  <div className="grid gap-2">
                    <select 
                      ...
                      onChange={(event) => updatePlan((plan) => relayoutPlan({ ...plan, edges: plan.edges.map((entry) => entry.id === edge.id ? { ...entry, fromNodeId: event.target.value } : entry) }))} 
                      className="w-full rounded-2xl border border-white/10 bg-[#101723] px-3 py-3 text-sm text-white outline-none"
                    >
                      {selectedPlan.nodes.map((node) => <option key={node.id} value={node.id}>{node.title}</option>)}
                    </select>
                    <select 
                      value={edge.toNodeId} 
                      onChange={(event) => updatePlan((plan) => relayoutPlan({ ...plan, edges: plan.edges.map((entry) => entry.id === edge.id ? { ...entry, toNodeId: event.target.value } : entry) }))} 
                      className="w-full rounded-2xl border border-white/10 bg-[#101723] px-3 py-3 text-sm text-white outline-none"
                    >
                      {selectedPlan.nodes.map((node) => <option key={node.id} value={node.id}>{node.title}</option>)}
                    </select>
                    <button 
                      type="button" 
                      onClick={() => updatePlan((plan) => relayoutPlan({ ...plan, edges: plan.edges.filter((entry) => entry.id !== edge.id) }))} 
                      className="rounded-2xl border border-white/10 px-3 py-2 text-sm text-white/70"
                    >
      ...
      case 'assignments':
        return (
          <div className="space-y-4">
            <p className="text-xs font-black uppercase tracking-[0.24em] text-white/72">Assignments</p>
            {selectedPlan ? (
                ...
                {filteredProfiles.map((profile) => {
                  ...
                  return (
                    <button 
                      ...
                      className={`w-full rounded-2xl border px-3 py-3 text-left text-sm ${active ? 'border-bjj-gold/35 bg-bjj-gold/10 text-white' : 'border-white/10 bg-white/[0.03] text-white/76'}`}
                    >
                      {profile.label}
                    </button>
                  )
  ...
  return (
    <div className={`${selectedPlan ? 'fixed inset-0 z-10 overflow-hidden' : 'space-y-5'}`}>
      {!selectedPlan ? (
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-col items-end gap-3">
            <div className="flex flex-wrap justify-end gap-2">
              <button type="button" onClick={createPlan} className="rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm font-semibold text-white/84">
                <Plus className="mr-2 inline h-4 w-4" />Neuer Plan
              </button>
            </div>
          </div>
        </div>
      ...
      {!selectedPlan && message ? (
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white/78">{message}</div>
      ) : null}
      ...
      {!selectedPlan ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {filteredPlans.map((plan) => (
              <button 
                ...
                type="button" 
                onClick={() => { setSelectedPlanId(plan.id); setSelectedTechniqueId(null) }} 
                className="overflow-hidden rounded-[1.6rem] border border-white/10 bg-[linear-gradient(180deg,rgba(19,25,36,0.96),rgba(12,16,24,0.95))] text-left transition hover:-translate-y-0.5 hover:border-bjj-gold/30"
              >
                <div 
        ...
        <div className="h-screen w-screen overflow-hidden">
          {/* Main Canvas - Full screen, no scrollbars */}
          <div
            ref={canvasRef}
            ...
            className="fixed inset-0 h-screen w-screen overflow-hidden bg-[linear-gradient(180deg,#0d111a,#090c13)]"
            style={{ cursor: isPanning ? 'grabbing' : 'grab', overscrollBehavior: 'none' }}
          >
              ...
              >
                ...
                {selectedPlan.nodes.map((node) => (
                  <button 
                    ...
                    className={`absolute rounded-[1.35rem] border p-4 text-left shadow-[0_18px_42px_rgba(0,0,0,0.18)] ${getStageTone(node.stage, node.id === selectedTechniqueId || node.id === selectedSourceNodeId)} ${connectMode && selectedSourceNodeId === node.id ? 'ring-2 ring-bjj-gold/70' : ''}`} 
                    style={{ left: ADMIN_CANVAS_PAD_X + node.x, top: ADMIN_CANVAS_PAD_Y + node.y, width: NODE_WIDTH[node.size], minHeight: NODE_HEIGHT[node.size] }}
                  >
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-[10px] font-black uppercase tracking-[0.18em] text-bjj-gold">{node.unlockPhase === 'core' ? 'A-Plan' : SIZE_LABELS[node.size]}</span>
                      <span className={`text-[10px] font-bold uppercase tracking-[0.16em] ${STAGE_META[node.stage].pill}`}>{STAGE_META[node.stage].title}</span>
                    </div>
          ...
          {/* Technique Details Panel - Bottom center */}
          {selectedTechnique ? (
            <div 
              className="pointer-events-auto fixed bottom-6 left-1/2 z-30 w-[600px] max-w-[90vw] -translate-x-1/2 rounded-[1.5rem] border border-white/10 bg-[linear-gradient(180deg,rgba(13,18,27,0.96),rgba(10,14,22,0.94))] p-5 shadow-[0_24px_60px_rgba(0,0,0,0.32)] backdrop-blur-md" 
            ...
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <button 
                  ...
                  className="inline-flex h-9 w-9 items-center justify-center rounded-2xl border border-white/10 bg-[#101723] text-white/70 transition hover:text-white"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                <input 
                  ...
                  onChange={(event) => updateTechnique((node) => ({ ...node, title: event.target.value }))} 
                  className="rounded-2xl border border-white/10 bg-[#101723] px-3 py-3 text-sm text-white outline-none" 
                />
                <input 
                  value={selectedTechnique.label} 
                  onChange={(event) => updateTechnique((node) => ({ ...node, label: event.target.value }))} 
                  className="rounded-2xl border border-white/10 bg-[#101723] px-3 py-3 text-sm text-white outline-none" 
                />
                <select 
                  value={selectedTechnique.stage} 
                  onChange={(event) => updateTechnique((node) => ({ ...node, stage: event.target.value as GameplanAdminNode['stage'] }))} 
                  className="rounded-2xl border border-white/10 bg-[#101723] px-3 py-3 text-sm text-white outline-none"
                >
                  <option value="position">position</option>
                  <option value="pass">pass</option>
                ...
                <select 
                  value={selectedTechnique.unlockPhase} 
                  onChange={(event) => updatePlan((plan) => relayoutPlan({
                    ...plan,
                  ...
                  className="rounded-2xl border border-white/10 bg-[#101723] px-3 py-3 text-sm text-white outline-none"
                >
                  <option value="core">core / A-Plan</option>
                  <option value="expansion">expansion</option>
                </select>
                <select 
                  value={selectedTechnique.size} 
                  onChange={(event) => updateTechnique((node) => ({ ...node, size: event.target.value as GameplanAdminNode['size'] }))} 
                  className="rounded-2xl border border-white/10 bg-[#101723] px-3 py-3 text-sm text-white outline-none"
                >
                  <option value="main">A-Plan gross</option>
                  <option value="branch">B-Plan mittel</option>
                ...
                <input 
                  type="number" 
                  min={1} 
                  value={selectedTechnique.unlockOrder ?? ''} 
                  onChange={(event) => updatePlan((plan) => relayoutPlan({
                    ...plan,
                ...
                />
                <select 
                  value={selectedTechnique.unlockParentNodeId ?? ''} 
                  onChange={(event) => updatePlan((plan) => relayoutPlan({
                    ...plan,
                ...
                >
                  ...
                  {selectedPlan.nodes.filter((node) => node.id !== selectedTechnique.id).map((node) => (
                    <option key={node.id} value={node.id}>{node.title}</option>
                  ))}
                </select>
                <button 
                  type="button" 
                  onClick={() => startConnectMode(selectedTechnique.id)} 
                ...
                >
                  {connectMode && selectedSourceNodeId === selectedTechnique.id ? 'Ziel auswaehlen...' : 'Mit anderer Technik verbinden'}
                </button>
                <button 
                  type="button" 
                  onClick={() => updatePlan((plan) => relayoutPlan({
                    ...plan,
                ...
                >
                  {selectedTechnique.requiresValidation ? 'Validation Pflicht: an' : 'Validation Pflicht: aus'}
                </button>
              </div>
            </div>
          ) : null}

          {/* Fixed Top Bar Controls */}
          <div className="fixed left-6 right-[320px] top-4 z-30 flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => { setSelectedPlanId(null); setSelectedTechniqueId(null); setActiveTab(null); }}
                className="rounded-2xl border border-white/10 bg-[rgba(18,23,33,0.96)] px-4 py-2.5 text-sm text-white/78 backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)]"
              >
                Zurück zur Übersicht
              </button>
              <div className="rounded-2xl border border-white/10 bg-[rgba(18,23,33,0.96)] px-4 py-2.5 text-sm font-semibold text-white/78 backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)]">
                {selectedPlan.title}
              </div>
              <button
                type="button"
                onClick={relayoutSelectedPlan}
                className="rounded-2xl border border-bjj-gold/25 bg-bjj-gold/10 px-4 py-2.5 text-sm text-bjj-gold backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)]"
              >
                Layout neu anordnen
              </button>
              {selectedTechnique ? (
                <button
                  ...
                  className="rounded-2xl border border-white/10 bg-[rgba(18,23,33,0.96)] px-4 py-2.5 text-sm text-white/68 backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)]"
                >
                  Technik entfernen
                </button>
              ) : null}
            ...
            <div className="flex items-center gap-2">
              {/* Zoom Controls */}
              <div className="flex items-center gap-1 rounded-2xl border border-white/10 bg-[rgba(18,23,33,0.96)] px-2 py-1.5 backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)]">
                <button type="button" onClick={() => adjustCanvasZoom(-0.1)} className="rounded-lg border border-white/10 p-1.5 text-white/72 hover:bg-white/[0.05]">
                  <Minus className="h-3.5 w-3.5" />
                </button>
                <span className="px-2 text-xs font-bold text-white/72">{Math.round(zoom * 100)}%</span>
                <button type="button" onClick={() => adjustCanvasZoom(0.1)} className="rounded-lg border border-white/10 p-1.5 text-white/72 hover:bg-white/[0.05]">
                ...
                <button
                  ...
                  className="ml-1 rounded-lg border border-white/10 px-2.5 py-1.5 text-xs text-white/72 hover:bg-white/[0.05]"
                >
                  Reset
                </button>
              </div>

              <button type="button" disabled={!selectedPlan || saving} onClick={() => void persist('duplicate')} className="rounded-2xl border border-white/10 bg-[rgba(18,23,33,0.96)] px-4 py-2.5 text-sm font-semibold text-white/84 disabled:opacity-40 backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)]">
                <Copy className="mr-2 inline h-4 w-4" />Duplizieren
              </button>
              {planDeleteConfirmId === selectedPlan.id ? (
                <>
                  <button
                    ...
                    className="rounded-2xl border border-white/10 bg-[rgba(18,23,33,0.96)] px-4 py-2.5 text-sm text-white/72 disabled:opacity-40 backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)]"
                  >
                    Abbrechen
                  </button>
                  <button
                    ...
                    className="rounded-2xl border border-red-500/35 bg-red-500/12 px-4 py-2.5 text-sm font-semibold text-red-300 disabled:opacity-40 backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)]"
                  >
                    Wirklich löschen
                  </button>
                </>
                ...
                <button
                  ...
                  className="rounded-2xl border border-red-500/25 bg-red-500/10 px-4 py-2.5 text-sm font-semibold text-red-300 disabled:opacity-40 backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)]"
                >
                  Plan löschen
                </button>
              )}
              <button
                ...
                className="rounded-2xl border border-emerald-400/30 bg-emerald-400/12 px-4 py-2.5 text-sm font-semibold text-emerald-300 disabled:opacity-40 backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)]"
              >
                <Save className="mr-2 inline h-4 w-4" />
                {saving ? 'Veröffentlicht...' : selectedPlan.status === 'published' ? 'Published' : 'Publish'}
              </button>
              <button
                type="button"
                disabled={!selectedPlan || saving}
                onClick={() => selectedPlan && void persist('save', selectedPlan)}
                className="rounded-2xl border border-bjj-gold/30 bg-bjj-gold/12 px-4 py-2.5 text-sm font-semibold text-bjj-gold disabled:opacity-40 backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)]"
              >
                <Save className="mr-2 inline h-4 w-4" />
          ...
          {/* Right Side Icon Tabs */}
          <div className="fixed right-4 top-20 bottom-20 z-40 flex flex-col items-center gap-2">
            {/* Plans Tab */}
            <button
              type="button"
              onClick={() => setActiveTab(activeTab === 'plans' ? null : 'plans')}
              className={`flex h-12 w-12 items-center justify-center rounded-2xl border backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)] transition ${
                activeTab === 'plans'
                  ? 'border-bjj-gold/40 bg-bjj-gold/20 text-bjj-gold'
                  : 'border-white/10 bg-[rgba(18,23,33,0.96)] text-white/72 hover:text-white'
              }`}
              title="Plans"
            >
            ...
            <button
              type="button"
              onClick={() => setActiveTab(activeTab === 'technique' ? null : 'technique')}
              className={`flex h-12 w-12 items-center justify-center rounded-2xl border backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)] transition ${
                activeTab === 'technique'
                  ? 'border-bjj-gold/40 bg-bjj-gold/20 text-bjj-gold'
                  : 'border-white/10 bg-[rgba(18,23,33,0.96)] text-white/72 hover:text-white'
            ...
            <button
              type="button"
              onClick={() => setActiveTab(activeTab === 'assignments' ? null : 'assignments')}
              className={`flex h-12 w-12 items-center justify-center rounded-2xl border backdrop-blur-md shadow-[0_14px_36px_rgba(0,0,0,0.26)] transition ${
          ...
          {activeTab && selectedPlan && (
            <div 
              className="fixed right-[72px] top-16 bottom-4 z-40 w-[320px] overflow-hidden rounded-[1.6rem] border border-white/10 bg-[linear-gradient(180deg,rgba(18,23,33,0.96),rgba(12,16,24,0.95))] shadow-[0_24px_60px_rgba(0,0,0,0.32)]"
              onMouseDown={(event) => event.stopPropagation()}
            >
              <div className="flex h-full flex-col">
                {/* Panel Header with close */}
                <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
                  <p className="text-xs font-black uppercase tracking-[0.24em] text-bjj-gold">
                    {activeTab === 'plans' && 'Plans'}
                    {activeTab === 'settings' && 'Settings'}
                    {activeTab === 'technique' && 'Technik'}
                  ...
                  <button
                    ...
                    className="inline-flex h-8 w-8 items-center justify-center rounded-xl border border-white/10 text-white/60 transition hover:text-white"
                  >
          ...
          {/* Message Toast */}
          {message ? (
            <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-2xl border border-white/10 bg-[rgba(18,23,33,0.98)] px-6 py-3 text-sm text-white/78 shadow-[0_24px_60px_rgba(0,0,0,0.32)]">
              {message}
            </div>
          ) : null}
        </div>
